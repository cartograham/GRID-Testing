...
output_file = os.path.join(thisdir,"output",orig_file.split(".")[0] + "_results." + orig_file.split(".")[1])
...
results_filteredtestplanfile = os.path.join(thisdir,"output",orig_file.split(".")[0] + "_filter." + orig_file.split(".")[1])
...
results_transposedtestplanfile = os.path.join(thisdir,"output",orig_file.split(".")[0] + "_transposed." + orig_file.split(".")[1])
f = csv.reader(open(results_filteredtestplanfile))
f_transposed = zip(*f)
f_transposed_list = []
count = len(f_transposed)
#Remove non-delimiting commas from the mix.
for item in f_transposed:
    thisrow = list(item)
    newrow = []
    for value in thisrow:
        newrow.append(value.replace(",", "- ")) #in case users use commas, we won't upset the comma-delimted format.
    f_transposed_list.append(newrow)
...
while i < count:
    ...
    f_output.write(testplan_lines[i].strip() + "," + ",".join(f_transposed_list[i][1:]) + "\n")
    i += 1
